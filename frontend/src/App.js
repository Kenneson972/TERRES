...
const GaleriePage = () => {
  ...
  const gallery = [
    ...
    { type: 'video', url: 'https://customer-assets.emergentagent.com/job_villarental/artifacts/murxouhh_VIDEO-2025-10-03-12-18-12.mp4', title: 'Visite virtuelle 1' },
    { type: 'video', url: 'https://customer-assets.emergentagent.com/job_villarental/artifacts/w2oio5ts_VIDEO-2025-10-03-12-18-11.mp4', title: 'Visite virtuelle 2' },
    { type: 'video', url: 'https://customer-assets.emergentagent.com/job_villarental/artifacts/6tp3d75s_VIDEO-2025-10-03-12-18-11.mp4', title: 'Visite virtuelle 3' },
    { type: 'video', url: 'https://customer-assets.emergentagent.com/job_villarental/artifacts/cbqcvbhm_VIDEO-2025-10-03-12-18-11.mp4', title: 'Visite virtuelle 4' }
  ];
  ...
  return (<div className="page-container"><Navigation /><section className="galerie-section luxury"><div className="container"><div className="section-header-luxury"><span className="luxury-subtitle">Découvrez</span><h2 className="section-title-luxury">Notre Villa d'Exception</h2><p className="section-description-luxury">Des espaces pensés pour votre confort et vos événements d'exception</p></div><div className="gallery-grid-luxury">{gallery.map((media, idx) => (<div key={idx} className="gallery-item-luxury" onClick={() => openLightbox(media)}><div className="gallery-item-overlay-luxury">{media.type === 'video' ? (<div className="video-thumbnail"><video src={media.url} muted preload="metadata" /><div className="play-button"><svg width="60" height="60" viewBox="0 0 60 60" fill="none"><circle cx="30" cy="30" r="28" stroke="white" strokeWidth="2" opacity="0.8"/><path d="M24 20L40 30L24 40V20Z" fill="white"/></svg></div></div>) : (<img src={media.url} alt={media.title} loading="lazy" />)}<div className="gallery-overlay"><div className="gallery-overlay-content"><h3>{media.title}</h3><p>Cliquez pour agrandir</p></div></div></div></div>))}</div></div></section><section className="features-section luxury"><div className="container"><h2 className="section-title-luxury">Caractéristiques Premium</h2><div className="features-grid-luxury"><div className="feature-card-luxury"><div className="feature-icon-luxury"><Users size={40} /></div><h3>Capacité 80 personnes</h3><p>Idéal pour vos événements privés et célébrations mémorables</p></div><div className="feature-card-luxury"><div className="feature-icon-luxury"><Home size={40} /></div><h3>Villa spacieuse</h3><p>Espaces intérieurs et extérieurs généreux pour votre confort</p></div><div className="feature-card-luxury"><div className="feature-icon-luxury"><Home size={40} /></div><h3>Piscine privée</h3><p>Profitez d'une piscine exclusive avec vue panoramique</p></div><div className="feature-card-luxury"><div className="feature-icon-luxury"><CheckCircle size={40} /></div><h3>Équipements haut de gamme</h3><p>Cuisine professionnelle, sonorisation, climatisation</p></div><div className="feature-card-luxury"><div className="feature-icon-luxury"><CheckCircle size={40} /></div><h3>Espaces modulables</h3><p>Configuration personnalisée selon vos besoins</p></div><div className="feature-card-luxury"><div className="feature-icon-luxury"><CheckCircle size={40} /></div><h3>Parking sécurisé</h3><p>Stationnement privé pour vos invités</p></div></div></div></section>{showLightbox && selectedMedia && (<div className="lightbox-overlay" onClick={() => setShowLightbox(false)}><div className="lightbox-content" onClick={(e) => e.stopPropagation()}><button className="lightbox-close" onClick={() => setShowLightbox(false)}><X size={32} /></button>{selectedMedia.type === 'video' ? (<video src={selectedMedia.url} controls autoPlay className="lightbox-media" />) : (<img src={selectedMedia.url} alt={selectedMedia.title} className="lightbox-media" />)}<div className="lightbox-title">{selectedMedia.title}</div></div></div>)}<Footer /><Toaster position="top-right" /></div>);
};
