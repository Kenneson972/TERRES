...
const HomePage = () => {
  ...
  return (<div className="page-container"><Navigation /><section className="hero-section"><video className="hero-video" autoPlay loop muted playsInline poster={heroImg}><source src="https://customer-assets.emergentagent.com/job_villarental/artifacts/rsa6tvb7_308498c1-b147-4585-aeff-9a79443d77b5.mov" type="video/quicktime" /><source src="https://customer-assets.emergentagent.com/job_villarental/artifacts/rsa6tvb7_308498c1-b147-4585-aeff-9a79443d77b5.mov" type="video/mp4" /></video><div className="hero-image" style={{ backgroundImage: `url(${heroImg})` }}></div><div className="hero-overlay"></div><div className="hero-content"><h1 className="hero-title">Terre d'Évasion - Votre Villa de Rêve en Martinique</h1><p className="hero-subtitle">Jusqu'à 80 personnes | Piscine | Événements | Séjours</p><div className="hero-buttons"><Button data-testid="decouvrir-formules-btn" size="lg" className="hero-btn-primary" onClick={() => navigate('/formules')}>DÉCOUVRIR NOS FORMULES</Button><Button data-testid="voir-calendrier-btn" size="lg" variant="outline" className="hero-btn-secondary" onClick={() => navigate('/calendrier')}>VOIR LE CALENDRIER</Button></div><div className="scroll-indicator"><div className="scroll-arrow"></div></div></div></section><Footer /><Toaster position="top-right" /></div>);
};
...
const GaleriePage = () => {
  ...
  return (<div className="page-container"><Navigation /><section className="galerie-section"><div className="container"><h2 className="section-title">Découvrez la Villa</h2><p className="section-subtitle">Des espaces pensés pour votre confort et vos événements</p><div className="gallery-grid">{gallery.map((img, idx) => (<div key={idx} className="gallery-item"><img src={img} alt={`Villa ${idx + 1}`} loading="lazy" /></div>))}</div></div></section><section className="features-section"><div className="container"><h2 className="section-title">Caractéristiques</h2><div className="features-grid"><div className="feature-item"><Users size={32} /><h3>Capacité 80 personnes</h3><p>Idéal pour événements et grands groupes</p></div><div className="feature-item"><Home size={32} /><h3>Villa spacieuse</h3><p>Espaces intérieurs et extérieurs généreux</p></div><div className="feature-item"><Home size={32} /><h3>Piscine privée</h3><p>Profitez d'une piscine exclusive</p></div></div></div></section><Footer /><Toaster position="top-right" /></div>);
};
